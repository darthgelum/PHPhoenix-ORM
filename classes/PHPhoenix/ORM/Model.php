...
class Model
{
	 ...
	 * <code>
	 * class App\Model\Fairy extends \PHPhoenix\ORM\Model { }
	 * </code>
	...
	public function __construct($phoenix)
	{
		...
		$this->conn = $phoenix->db->get($this->connection);
		$this->query = $this->conn->query('select');
		$this->model_name = strtolower(get_class($this));
		$this->model_name = str_ireplace($this->phoenix->app_namespace."Model\\", '', $this->model_name);
	 ...
	 *
	 * @return array Relationship definitions used by \PHPhoenix\ORM\Result
		...
		if (!empty($this->_with))
		{
			...
			foreach ($this->_with as $target) {
				...
				foreach ($rels as $key => $rel_name) {
					...
					$rel = $this->phoenix->arr($model_rels, $rel_name, false);
					...
					$rel_model = $this->phoenix->orm->get($rel['model']);
	 ...
	 *
	 * @return \PHPhoenix\ORM\Result Returns ORM Result that you can use in a 'foreach' loop.
	 */
	public function find_all()
	{
		$paths = $this->prepare_relations();
		return $this->phoenix->orm->result($this->model_name, $this->query->execute(), $paths);
	}
	 ...
	 *
	 * @return \PHPhoenix\ORM\Model Found item or new object of the current model but with loaded() flag being False
	 */
	 ...
	 *
	 * @return \PHPhoenix\DB\Query A clone of the current query builder
	 */
	...
	public function __isset($property) {
		...
		if ($target = $this->phoenix->arr($relations, $property, false))
			return true;
		return false;		
	}
	...
	public function __get($column)
	{
		...
		{	
			return $this->extension_instances[$column] = 
				$this->phoenix->orm->extension($this->extensions[$column], $this);
		}
		
		$relations = array_merge($this->has_one, $this->has_many, $this->belongs_to);
		if ($target = $this->phoenix->arr($relations, $column, false))
		{
			$model = $this->phoenix->orm->get($target['model']);
	 ...
	 * @param \PHPhoenix\ORM\Model    $model    ORM item to create a relationship with
	 * @return void
	...
	public function add($relation, $model)
	{
		...
		$rel = $this->phoenix->arr($rels, $relation, false);
		if (!$rel)
		{
	 ...
	 *
	 * @param string   $relation Name of the relationship
	 * @param \PHPhoenix\ORM\Model    $model    ORM item to remove relationship with. Can be omitted for 'belongs_to' relationships
	 * @return void
	...
	{
		$cache = &$this->phoenix->orm->column_cache;
	...
	* @return \PHPhoenix\ORM\Model Returns itself
	*/
	 ...
	 *
	 * @return \PHPhoenix\ORM\Model Returns self
	 */
