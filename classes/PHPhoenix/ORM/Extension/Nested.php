<?php
namespace PHPhoenix\ORM\Extension;
 ...
 * @package ORM
 */
class Nested extends \PHPhoenix\ORM\Extension{
	
	/**
	 * Method to modify each database query.
	 * Useful if you need to add an additional special condition.
	 *
	 * @param \PHPhoenix\DB\Query $query Query to modify
	 * @return \PHPhoenix\DB\Query Modified Query
	 */
	...
	protected function max_rpos_query() {
		return $this->modify_query(
					$this->model->conn->query('select')
						->fields($this->phoenix->db->expr('MAX(rpos) as rpos'))
	...
	protected function reverse_children_pos_query() {
		return $this->modify_query(
					$this->model->conn->query('update')
						->table($this->model->table)
						->data(array(
							'lpos' => $this->phoenix->db->expr('0-lpos'),
							'rpos' => $this->phoenix->db->expr('0-rpos')
						))
	...
	protected function pad_rpos_query($lpos, $width) {
		return $this->modify_query(
					$this->model->conn->query('update')
						->table($this->model->table)
						->data(array(
							'rpos' => $this->phoenix->db->expr('rpos + '.$width)
						))
					->where('rpos', '>=', $lpos)
				);
	}
	...
	protected function pad_lpos_query($lpos, $width) {
		return $this->modify_query(
					$this->model->conn->query('update')
						->table($this->model->table)
						->data(array(
							'lpos' => $this->phoenix->db->expr('lpos + '.$width)
						))
						->where('lpos', '>=', $lpos)
				);
	}
	...
	protected function update_reversed_query($pos_offset, $depth_offset) {
		return $this->modify_query(
					$this->model->conn->query('update')
						->table($this->model->table)
						->data(array(
							'lpos' => $this->phoenix->db->expr("0 - lpos + $pos_offset"),
							'rpos' => $this->phoenix-> db->expr("0 - rpos + $pos_offset"),
							'depth' => $this->phoenix->db->expr("depth + $depth_offset")
						))
					->where('rpos', '<', 0)
				);
	}
	 ...
	 * 
	 * @param \PHPhoenix\ORM\Model $parent Parent to append the node to.
	 ...
	 * 
	 * @param \PHPhoenix\ORM\Model $parent Parent to append the node to. If it's an empty model it will be ignored.
	 * @param bool $append_to_beginning  Prepend node to the beginning of children list.
	 *
	 * @return \PHPhoenix\ORM\Model Current Model
	 */
	 ...
	 * 
	 * @param \PHPhoenix\ORM\Model $parent Parent to move child nodes to.
	 * @param bool $append_to_beginning  Prepend node to the beginning of children list.
	 *
	 * @return \PHPhoenix\ORM\Model Current Model
	 ...
	 * of the current node.
	 * 
	 * @return \PHPhoenix\ORM\Model Model with conditions set
	 * @throw \Exception If associated model is not loaded
	 */
	public function children() {
		if (!$this->model->loaded())
			throw new \Exception("The model is not loaded");
		return $this->phoenix->orm->get($this->model->model_name)
				->where('lpos', '>', $this->model->lpos)
				->where('rpos', '<', $this->model->rpos)
				->order_by('lpos', 'asc');
	}
