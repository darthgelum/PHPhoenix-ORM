...
class ORM_Extension_Nested_Test extends PHPUnit_Framework_TestCase
{
	...
	protected function setUp()
	{
		...
		$this->phoenix = $this->getMock('\PHPhoenix\Phoenix',array('find_file'));
		$this->phoenix->expects($this->any())
                 ->method('find_file')
                 ->will($this->returnValue($this->conf_file));
		$this->phoenix->db = new \PHPhoenix\DB($this->phoenix);
		$this->phoenix->orm = new \PHPhoenix\ORM($this->phoenix);
				 
		$this->phoenix->config->set('db.orm.connection', 'sqlite:'.$this->db_file);
		$this->phoenix->config->set('db.orm.driver', 'PDO');
	}
	
	protected function tearDown()
	{	
		$db = $this->phoenix->db->get('orm');
		$db->conn = null;
		unlink($this->db_file);
	...
	public function testTree() {
		...
		//print_r($this->phoenix-> db->get('orm')->execute("SELECT * from nested")->as_array());
		...
		$n = new \Model\Nested($this->phoenix);
		$except = false;
		try {
	...
	protected function refreshNodes() {
		$nodes = new \Model\Nested($this->phoenix);
