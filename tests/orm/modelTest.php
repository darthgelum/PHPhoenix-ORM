...
class ORM_Model_Test extends PHPUnit_Framework_TestCase
{
	...
	protected function setUp()
	{
		...
		$this->fairies = new \Model\Fairy($this->phoenix);
		$this->trees = new \Model\Tree($this->phoenix);
	}
	...
	public function testFind_all()
	{
		...
		$this->assertEquals('Trixie', $this->phoenix->orm->get('fairy')->where('id', 2)->find_all()->current()->name);
	}
	...
	public function testCount_all()
	{
		...
		$this->assertEquals(2, $this->phoenix->orm->get('fairy')->where('id', 2)->protects->count_all());
	}
	...
	public function test__get()
	{
		...
		$this->assertEquals('Trixie', $this->phoenix->orm->get('fairy')->find()->friends->find()->name);
	}
	...
	{
		$fairy = $this->phoenix->orm->get('fairy')->find();
		$fairy->add('tree', $this->phoenix->orm->get('tree')->where('id', 2)->find());
		$fairy->save();
		$this->assertEquals('Willow', $this->phoenix->orm->get('fairy')->find()->tree->name);

		$fairy = $this->phoenix->orm->get('fairy');
		$fairy->add('tree', $this->phoenix->orm->get('tree')->where('id', 2)->find());
		$fairy->id = 3;
		$fairy->save();
		$fairy = $this->phoenix->orm->get('fairy', 3);
		$this->assertEquals('Willow', $fairy->tree->name);

		$tree = $this->phoenix->orm->get('tree')->find();
		$fairy = $this->phoenix->orm->get('fairy');
		$tree->fairy = $fairy;
		$fairy->id = 4;
		$fairy->save();
		$this->assertEquals('Oak', $this->phoenix->orm->get('fairy', 4)->tree->name);


		$fairy = $this->phoenix->orm->get('fairy')->where('id', 2)->find();
		$fairy->add('friends', $this->phoenix->orm->get('fairy')->where('id', 1)->find());
		$this->assertEquals('Tinkerbell', $this->phoenix->orm->get('fairy')->where('id', 2)->find()->friends->find()->name);
	}
	...
	public function test__set()
	{
		...
		$fairy = $this->phoenix->orm->get('fairy')->where('id', 2)->find();
		$fairy->friends = $this->phoenix->orm->get('fairy')->where('id', 1)->find();
		$this->assertEquals('Tinkerbell', $this->phoenix->orm->get('fairy')->where('id', 2)->find()->friends->find()->name);

		$this->phoenix->orm->get('tree')->where('id', 2)->find()->fairy = $this->phoenix->orm->get('fairy')->find();
		$this->phoenix->orm->get('tree')->find()->fairy = $this->phoenix->orm->get('fairy')->where('id', 2)->find();

		$this->assertEquals('Trixie', $this->phoenix->orm->get('tree')->find()->fairy->name);
	}
	...
	public function testRemove()
	{
		...
		$fairy->remove('friends', $this->phoenix->orm->get('fairy')->where('id', 2)->find());
		$this->assertEquals(false, $fairy->friends->find()->loaded());

		$fairy = $this->phoenix->orm->get('fairy')->where('id', 2)->find();
		$fairy->remove('protects', $this->phoenix->orm->get('tree')->where('id', 1)->find());
	...
	public function testInit_columns()
	{
		...
		$fairy = $this->phoenix->orm->get('fairy', 0)->init_columns();
		$this->assertEquals(null, $fairy->id);
		$this->assertEquals(null, $fairy->name);
		$this->assertEquals(null, $fairy->tree_id);

		$fairy = $this->phoenix->orm->get('fairy', 0)->init_columns( array('name' => 'Phoenix', 'tree_id' => 0) );
		$this->assertEquals(null, $fairy->id);
		$this->assertEquals('Phoenix', $fairy->name);
	...
	/**
	 * @covers $this->phoenix->orm->get
	 * @todo   Implement testFactory().
	 */
	public function testFactory()
	{
		$this->assertEquals('fairy', $this->phoenix->orm->get('fairy')->model_name);
	}
	
	public function testModel_key() {
		$this->assertEquals('forest_fairy_id', $this->phoenix->orm->get('fairy')->model_key('forest\fairy'));
	}
	
	public function testNamespaceTable() {
		$this->assertEquals('forest_fairies', $this->phoenix->orm->get('forest\fairy')->table);
	}
